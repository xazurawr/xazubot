...
public class Bot {
    ...
    static YandexMusicClient yandexMusicClient;
    ...
    public static void init() {
            ...
            if (event.getMessage().equals(Config.botCommand)) {
                ...
                MediaSessionHelper.getMediaSessions(new MediaSessionHelper.MediaSessionsCallback() {
                    ...
                    public void onResult(String[] mediaSessions) {
                        ...
                        if (Config.integrations.containsKey("yandexApi")
                                && Config.integrations.get("yandexApi")
                                && !Config.yandexToken.isEmpty()) {
                            String trackId = yandexMusicClient.getCurrentTrackIdAsync().join();
                            TrackInfo trackInfo = yandexMusicClient.getTrackInfoAsync(trackId).join();

                            message.append(trackInfo.title());
                            message.append(" - ");
                            message.append(trackInfo.artist());
                            message.append(" music ");
                        }
        ...
        if (!Config.yandexToken.isEmpty()) {
            Logs.log("Инициализация клиента YandexMusic...");
            yandexMusicClient = new YandexMusicClient();
            yandexMusicClient.setToken(Config.yandexToken);
        }
    }
